...
# # This depracated library is required to encode and upload Files.
# # https://github.com/JeremyGrosser/python-poster
# try:
#     from poster.encode import multipart_encode, MultipartParam
# except ImportError:
#     logging.warning("Cannot import python-poster library, \
#                       will not be able to "
...
api_oauth = 'https://app.clio.com/oauth/'
...
class ClioClient(object):
    def __init__(self, access_token):
        ...
        '''
        #  Access token is an element in the json
    ...
    #         self.content_type = ClioClient._to_data(content_type,
    #                               "content_type")
    ...
    class OAuth(object):
        ...
        def authorize_url(self, redirect_uri, state=''):
            ...
            '''
            return api_oauth + 'authorize?' + urllib.parse.urlencode({
                'response_type': 'code',
        ...
        def get_token(self, code, redirect_uri):
            ...
            post_args = urllib.parse.urlencode(d).encode("utf-8")
            return json.loads(urllib.request.urlopen(api_oauth + '/token',
                                                     data=post_args).read())
    ...
    def _to_keyvalue(cls, q, _parent_key=None):
        ...
        '''
        # if isinstance(q, (int, str, cls.FileUpload)):
    ...
    def _call(self, endpoint, method="GET", *args, **kwargs):
        ...
        if method not in ["GET", "POST", "DELETE", "PUT"]:
            raise Exception(
                "Expecting a GET or POST request, not: %s" % method)
        ...
        if 'documents' in endpoint:
            if method == "GET":
                ...
                req = urllib.request.Request(url + "?" + urlargs)
            else:
            ...
            if kwargs:
                jsonargs = bytes(json.dumps(kwargs), 'utf-8')
                self.deb("JSON args:\n%s", jsonargs)
                req = urllib.request.Request(url, jsonargs)
